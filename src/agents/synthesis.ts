/**
 * FactForge — Agent 4: Synthesis Agent
 *
 * Responsibilities:
...
2. Generate a complete Markdown research report with:
   ...
   - IMPORTANT: Any claim that was re-researched OR has "contradicted"/"unverifiable" status MUST be clearly flagged with WARNING
   - Methodology section
   - Disclaimer
...
${c.attemptCount > 0 ? `- **Re-researched:** ${c.attemptCount}x through feedback loop` : ""}`;
    })
    .join("\n\n");
...
This report was generated by FactForge's 4-agent pipeline: Research Agent (Tavily search + claim extraction), Verification Agent (independent cross-checking), Contradiction Detector (pairwise + source fidelity checks), and Synthesis Agent (confidence scoring + report generation).

---

*Disclaimer: This is AI-generated research. Verify critical claims independently before making decisions.*`;
}
